...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.C) && jumpNum < 2 && !isRoll && isAlive) // C키로 점프
        {
            jumpNum++;
            if (vec.x != 0)
                scaleVec.x = vec.x;
        ...
        if (Input.GetKeyDown(KeyCode.Z) && attackNum < 2 && !isRoll && isAlive) // z키로 공격 
        {
        ...
        if (Input.GetKeyDown(KeyCode.X) && !isAttack1 && !isAttack2 && !isRoll && !isJump && isAlive && rollCool <= 0f)
            StartCoroutine(PlayRoll()); // x키로 회피 실행
